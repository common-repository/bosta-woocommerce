...
function checkValidApiKey($bostaApiKey) {
	if($bostaApiKey == null){
		return false;
	}
	
	$url = BOSTA_ENV_URL . '/businesses/' . esc_html($bostaApiKey). '/info';
	...
	$response_body = wp_remote_retrieve_body($business_result);

	$response_code = wp_remote_retrieve_response_code($business_result);

	if (is_wp_error($business_result) || $response_code!==200 || $response_body =='null' ) {
		return false;
	}
	...
	if (!class_exists('bosta_Shipping_Method')) {
		class bosta_Shipping_Method extends WC_Shipping_Method
		{
			...
			public function process_admin_options()
			{
				...
				} elseif (!checkValidApiKey($apikey)) {
